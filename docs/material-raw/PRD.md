...
| FR-006 | Fecha de negocio (`D4` y `db_materialrow!A`) nunca se valida ni se convierte en Apps Script; se persiste tal cual `Date` nativo. `America/La_Paz` solo para `timestamp`/`creado` de auditoría. | Must |
...
| J | `timestamp` | DATETIME | `2026-09-19 22:10:00` | Auditoría — `America/La_Paz` |
...
| EC-04 | Auditoría | `J` usa `America/La_Paz` (`Utilities.formatDate(new Date(), "America/La_Paz", "yyyy-MM-dd HH:mm:ss")`). Nunca para `fecha`. |
