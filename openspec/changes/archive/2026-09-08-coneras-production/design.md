...
| Hydrate filter `fecha` first, then `turno`+`maquina` | Filter all predicates together | PRD FR-001: optimizes for mobile date-first scan; date normalization (`yyyy-MM-dd` plain calendar string, timezone-agnostic) avoids drift. `America/La_Paz` only for `creado`/`actualizado` audit timestamps. |
...
| Native `SUMPRODUCT` in `dashboard` (`F7:F16`, `H6:H36`/`I6:I36`, pivots `K/S/V`); single delete-guard `Ui.alert` for batch | `QUERY`/`SUMAR.SI`/Script aggregation; per-row alert | Decouples aggregation from form geometry; `SUMPRODUCT` avoids `QUERY` locale/header issues and keeps all ranges `L2:L`/`F2:F`/`B2:B`; one alert listing `Nº` preserves UX for multi-delete. |
| `es-BO` verbatim formulas via `valueRenderOption=FORMULA` | English formulas / `setValue` | Spreadsheet locale is `es-BO`; `SI`/`ESNUMERO`/`MAX`/`SUMA` and English `IF`/`TODAY`/`DATE`/`EOMONTH` in `SUMPRODUCT` must survive locale separator handling. |
...
const CONERAS_CONFIG = Object.freeze({
  ...
  FORMULAS: Object.freeze({
    ...
    DASHBOARD_F7: '=SUMPRODUCT((db_coneras!L2:L)*(db_coneras!F2:F=E7)*(db_coneras!B2:B>=IF(...))*(...((($B$7="Todos")+(db_coneras!M2:M=$B$7))>0)...))' // per-title F7:F16, daily H/I, pivots K/S/V use same SUMPRODUCT pattern
  })
});
```

Helpers: `conerasGetSheet_(ss, key)`, `conerasParseA1_(a1)`, `conerasNormalizeFecha_(v)` → `yyyy-MM-dd` plain calendar string, timezone-agnostic (`getFullYear`/`getMonth`/`getDate` without `Utilities.formatDate`; `America/La_Paz` only for audit timestamps), `conerasBuildId_(fecha,turno,maquina,nro)` (maquina upper no-spaces, nro zero-pad 2).
...
| `Ingest.gs` | Installable `conerasOnEdit(e)` router: if `e.range==I8 && TRUE→guardarTurno()+reset FALSE (~1 s)`; else if `e.range in {E5,G4,C5} && sheet==conera → conerasHydrate_()` (normalize `E5` datetime→plain `yyyy-MM-dd` calendar string, timezone-agnostic). Ignores `B8:H22` edits. |
...
E5/G4/C5 onEdit (installable) ─▶ conerasOnEdit(e) ─▶ conerasHydrate_()
   ...
   filter db_coneras Map by plain fecha(yyyy-MM-dd, timezone-agnostic) → turno → maquina
...
sequenceDiagram
    ...
    E->>C: conerasHydrate_()
    C->>S: get E5,G4,C5,G5 + normalize fecha (datetime→yyyy-MM-dd plain string, timezone-agnostic)
    C->>DB: batch getValues A:P → Map id->row (filter fecha first, plain string match)
...
sequenceDiagram
    ...
    alt invalid
        ...
        alt deletes non-empty
         ...
         P->>DB: upsert batch (append or overwrite F:G/H:K/L/M, preserve N creado, set O/P actualizado/editado_por America/La_Paz; fecha B remains plain string / Date@noon display artifact)
        DB-->>G: flush
        G->>L: releaseLock()
...
- PK `db_coneras`: `(fecha, turno, maquina, descarga_nro)` → `id = yyyy-MM-dd-TURNO-MAQUINANORM- NN` (upper, no spaces, zero-pad 2). Columns A:P frozen: `id,fecha,turno,maquina,descarga_nro,titulo,operador,peso_bruto,usos,peso_canilla,peso_tacho,peso_neto,supervisor,creado,actualizado,editado_por`. `fecha` is plain calendar string `yyyy-MM-dd` (display `dd/MM/yyyy` via `numberFormat`; persistence may store a `Date@12:00` display artifact for `SUMPRODUCT` compatibility, but PK/logic remains the plain string; `America/La_Paz` only for `creado`/`actualizado`); `turno` STRING.
...
- Verbatim formulas (`valueRenderOption=FORMULA`): `H8=SI(ESNUMERO(D8),MAX(0,D8-(E8*F8)-G8),"")`, `H23=SUMA(H8:H22)`, `E4=SI(ESNUMERO(C4),C4*dashboard!B7,"")`, dashboard `F7:F16`/`H6:H36`/`I6:I36`/pivots use `SUMPRODUCT((L2:L)*(F2:F=E7)*(B>=IF(...))*((($B$7="Todos")+(M2:M=$B$7))>0)...)` with English `IF`/`TODAY`/`DATE`/`EOMONTH`, no `QUERY`.

## Dashboard — `SUMPRODUCT` Native + Periodo Logic (reconciled from `QUERY`)

- `dashboard` totals: `F7:F16` per-title `=SUMPRODUCT((L2:L)*(F2:F=E7)*(B>=IF(...))*(B<IF(...))*((($B$7="Todos")+(M2:M=$B$7))>0)*...)` with English `IF`/`TODAY`/`DATE`/`EOMONTH` (no `QUERY`). Daily `H6:H36`/`I6:I36` and pivots `K/S/V` use the same `SUMPRODUCT` criteria pattern. Predicates with `((Todos+M)>0)` omit filter when `Todos`. No Script writes dashboard.
- Periodo semantics (all dates `America/La_Paz` via `TODAY()`): `Fecha →` date criteria yields empty (`IF(B6="","",...)*` → `SUMPRODUCT` 0/EC-09); `Semana → B ∈ [TODAY()-6, TODAY()]` (7-day rolling); `Mes → B ∈ [DATE(YEAR(TODAY()),MONTH(TODAY()),1), EOMONTH(TODAY(),0)]`. `Semana`/`Mes` ignore `Fecha` picker entirely (EC-10). Charts: primary `Peso Neto por Titulo` (bars on `E5:F16`) always visible; secondary `Evolucion diaria` (`H5:I36` line + `AREA`/`LINE`/`COLUMN` pivots) visible only `Semana`/`Mes`, hidden/minimized for `Fecha` (EC-12: `E4` invalid efficiency → `""`, no block).
- `dashboard!B7` (now `B8` in shipped layout) global efficiency factor editable; `E4` derivation `SI(ESNUMERO(C4),…)` handles blank/non-numeric without blocking save. Implementation is `SUMPRODUCT` (design originally `QUERY`, migrated via PR 3 commit `refactor: simplify native dashboard filters`, `fix dashboard formulas locale`).
...
| `E5` datetime | Normalize to plain `yyyy-MM-dd` calendar string, timezone-agnostic, before PK (EC-07); `America/La_Paz` only for `creado`/`actualizado`. |
...
| E2E | Menu/checkbox save, hydrate via `E5`/`G4`/`C5` (fecha-first), `Re-sincronizar` fallback, dashboard `SUMPRODUCT` Fecha/Semana/Mes + chart visibility (5 charts), mobile `I8` | Authenticated walkthrough on COPY only; verify displayValue 2 dec, `F7:F16` totals and `H/I` daily, 5-chart visibility, native validation rejection. Never prod. |
