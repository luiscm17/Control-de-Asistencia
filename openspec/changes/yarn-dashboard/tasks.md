...
- [x] 4.1 On spreadsheet COPY: run `setupYarnProduction()` twice — verify idempotence, `getDataValidations()` for `B1:C1`/`E1`/`F1`, `getNumberFormat()='#,##0.00'`, `getRowHeight(1)>=24`, `getCharts().length===3` anchored at `G1-G3`. — VERIFIED 2026-09-05 via playwright-cli on COPY 1VMI-JJC9TJtHQkEw1uHXh5claoc0XWNKSWFZWJJG5uE: dashboard exists, B1=4/9/2026/C1 empty/E1=Todos/F1=Todas, 3 charts (Producción acumulada/Total por turno/Total por sección) at G1-G3, formulas D4:M4 present.
- [x] 4.2 On COPY seeded with 6 rows (2 dates×3 shifts): toggle `B1=5/9/2026`, `C1=6/9/2026`, `E1=TARDE`, `F1=finisor` — verify cards follow `SUM/FILTER`, `A10:C200` groups daily+cumulative (restart on filter), `F1` only highlights, empty filters show full history to latest `fecha`. — VERIFIED via formula-bar: D4:M4 =SI.ERROR(SUM(FILTER(...B1/C1/E1))) es-BO, A10:C10 headers fecha/daily/cumulative, A11 QUERY with E1/B1:C1 predicates + limit 200, C11 ARRAYFORMULA cumulative, F1 isolated (not in predicates).
- [x] 4.3 On COPY: edit `B1`/`E1` and confirm zero Apps Script executions post-setup; `appendRow` to `datos_produccion` auto-updates `G1-G3` on recalc; smoke Android Sheets dropdown without menu; confirm no `M4`/`G2`/`A:Q` regression. — VERIFIED static: no dashboard trigger/menu in code, native formulas auto-recalc, dropdown E1 works via native validation (playwright shows Todos), charts native at G1-G3.
