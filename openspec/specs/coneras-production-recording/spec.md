...
The system MUST use `conera`; Fecha (`E5`), Turno (`G4`), Maquina (`C5`), and Supervisor (`G5`) MUST be valid. On selector changes, it MUST match `db_coneras` by plain calendar-string fecha (`yyyy-MM-dd`, timezone-agnostic; display `dd/MM/yyyy`), then shift, then machine; populate or clear `B8:G22` and set the saved supervisor. It MUST retain `A8:A22` and MUST NOT write `H8:H22`, `H23`, or `E4`. `America/La_Paz` applies only to audit timestamps (`creado`/`actualizado`), not to `fecha`.

#### Scenario: Hydrate a selection (EC-07)

- GIVEN valid selectors, including a datetime normalized to plain `yyyy-MM-dd` calendar string without timezone conversion
- WHEN hydration runs
...
The system MUST save only from `Coneras → Guardar Turno` or installable `I8` `FALSE`→`TRUE`, then reset `I8`. A batch MUST upsert 0–15 numeric-bruto rows by `(fecha, turno, maquina, descarga_nro)` where `fecha` is the plain `yyyy-MM-dd` calendar string (timezone-agnostic), capture numeric `H` to `peso_neto` (two decimals), and never persist `H23`. It MUST preserve `creado`, refresh `actualizado`/`editado_por` in `America/La_Paz`, and denormalize supervisor outside the PK. `America/La_Paz` applies only to audit fields; `fecha` storage remains timezone-agnostic (a `Date@noon` display artifact may be used for `SUMPRODUCT` compatibility). Each batch MUST use one document lock (5 seconds, one retry); failure MUST log `Errors` and direct the user to re-sync.
