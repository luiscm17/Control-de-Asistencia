...
function guardarTodo() {
  ...
  var lock = LockService.getDocumentLock();
  if (!yarnInventoryAcquireLock_(lock)) {
    yarnInventoryLogError_(YARN_INVENTORY_CONFIG.ERRORS.LOCK_TIMEOUT, 'Document lock exhausted after one retry.', 'guardarTodo', ss);
    ss.toast('\u23f3 Registro ocupado \u2014 reintentando\u2026', 'Inventario', 3);
    Utilities.sleep(300);
    ss.toast('\u274c Error \u2014 use Re-sincronizar', 'Inventario', 5);
    return { saved: false, reason: YARN_INVENTORY_CONFIG.ERRORS.LOCK_TIMEOUT };
  }
  var madejerasRes = null;
  var lotesRes = null;
  var madejerasSnapshot = null;
  var lotesSnapshot = null;
  try {
    yarnInventoryEnsureDbSheets_(ss);

    // Madejeras first
    madejerasSnapshot = yarnInventoryReadMadejerasSnapshot_(ss);
    if (!madejerasSnapshot.valid) {
      yarnInventoryLogError_(madejerasSnapshot.errorCode, madejerasSnapshot.reason, madejerasSnapshot.range, ss);
      ss.toast(madejerasSnapshot.message + ' (Madejeras \u2014 Lotes sigue)', 'Inventario', 5);
      madejerasRes = { saved: false, reason: madejerasSnapshot.errorCode };
    } else {
      var stateM = yarnInventoryLoadState_(ss);
      var planM = yarnInventoryBuildMadejerasPlan_(madejerasSnapshot, stateM);
      for (var wm = 0; wm < planM.warnings.length; wm++) ss.toast(planM.warnings[wm], 'Inventario', 5);
      if (planM.plans.length === 0) {
        ss.toast('\u26a0\ufe0f Madejeras: sin filas v\u00e1lidas \u2014 se omite', 'Inventario', 5);
        madejerasRes = { saved: false, reason: 'empty_form' };
      } else {
        yarnInventoryApplyMadejerasPlan_(ss, planM);
        // Reload state for lotes after madejeras writes (inserts change lastRow)
        SpreadsheetApp.flush();
        madejerasRes = { saved: true, count: planM.plans.length, plan: planM, snapshot: madejerasSnapshot };
      }
    }

    // Lotes second (always attempt even if madejeras failed — partial not rolled back)
    lotesSnapshot = yarnInventoryReadLotesSnapshot_(ss);
    if (!lotesSnapshot.valid) {
      yarnInventoryLogError_(lotesSnapshot.errorCode, lotesSnapshot.reason, lotesSnapshot.range, ss);
      ss.toast(lotesSnapshot.message + ' (Lotes)', 'Inventario', 5);
      lotesRes = { saved: false, reason: lotesSnapshot.errorCode };
    } else {
      var stateL = yarnInventoryLoadState_(ss);
      var planL = yarnInventoryBuildLotesPlan_(lotesSnapshot, stateL);
      for (var wl = 0; wl < planL.warnings.length; wl++) ss.toast(planL.warnings[wl], 'Inventario', 5);
      if (planL.plans.length === 0) {
        ss.toast('\u26a0\ufe0f Lotes: sin filas v\u00e1lidas \u2014 se omite', 'Inventario', 5);
        lotesRes = { saved: false, reason: 'empty_form' };
      } else {
        yarnInventoryApplyLotesPlan_(ss, planL);
        SpreadsheetApp.flush();
        lotesRes = { saved: true, count: planL.plans.length, plan: planL, snapshot: lotesSnapshot };
      }
    }

    // Final toast — partial warning if one succeeded and one failed
    var editor = yarnInventoryEditorEmail_();
    if (madejerasRes && madejerasRes.saved && lotesRes && lotesRes.saved) {
      // Use madejeras snapshot fecha+turno for lotes? They may differ — report both.
      var fechaDisp = madejerasSnapshot ? madejerasSnapshot.fechaKey : (lotesSnapshot ? lotesSnapshot.fechaKey : '');
      var turnoDisp = madejerasSnapshot ? madejerasSnapshot.turno : (lotesSnapshot ? lotesSnapshot.turno : '');
      ss.toast('\u2705 Guardado: ' + fechaDisp + ' ' + turnoDisp + ' \u2014 ' + madejerasRes.count + ' madejeras, ' + lotesRes.count + ' lotes por ' + editor, 'Inventario', 5);
      return { saved: true, madejeras: madejerasRes, lotes: lotesRes };
    }
    if ((madejerasRes && madejerasRes.saved) || (lotesRes && lotesRes.saved)) {
      var okM = madejerasRes && madejerasRes.saved ? madejerasRes.count + ' madejeras' : 'madejeras fall\u00f3';
      var okL = lotesRes && lotesRes.saved ? lotesRes.count + ' lotes' : 'lotes fall\u00f3';
      ss.toast('\u26a0\ufe0f Guardado parcial: ' + okM + ', ' + okL + ' por ' + editor + ' \u2014 revis\u00e1 Errors', 'Inventario', 7);
      if (madejerasRes && !madejerasRes.saved) yarnInventoryLogError_('partial_save', 'Madejeras failed in guardarTodo', 'madejeras', ss);
      if (lotesRes && !lotesRes.saved) yarnInventoryLogError_('partial_save', 'Lotes failed in guardarTodo', 'lotes', ss);
      return { saved: false, partial: true, madejeras: madejerasRes, lotes: lotesRes };
    }
    // Both failed — already toasted per form
    return { saved: false, madejeras: madejerasRes, lotes: lotesRes };

  } catch (e) {
    yarnInventoryLogError_('execution_failure', e && e.message ? e.message : String(e), 'guardarTodo', ss);
    ss.toast('\u274c Error \u2014 use Re-sincronizar', 'Inventario', 5);
    throw e;
  } finally {
    try { lock.releaseLock(); } catch (ignore) {}
